...
pii getans(char c, int pos){
    pii val = {B0, B1};

    for(int i = 0 ; i < n ; i++){
        if(s[i][pos] == c){
            if(val.first * ac[i].second < ac[i].first * val.second){
                val = ac[i];
            }
        }else{
            if(val.first * wa[i].second < wa[i].first * val.second){
                val = wa[i];
            }
        }
    }

    auto g = gcd(val.first, val.second);
    ...
    for(int test = 1 ; test <= t ; test++){
        ...
        for(int i = 0 ; i < q ; i++){
            ...
            if(r1.first * r2.second > r2.first * r1.second){
                BigInt l = lcm(r1.second, ans.second);
                ans = {ans.first * (l / ans.second) + r1.first * (l / r1.second), l};
                // cout << "T";
            }else{
                BigInt l = lcm(r2.second, ans.second);
                ans = {ans.first * (l / ans.second) + r2.first * (l / r2.second), l};
                // cout << "F";
            }

            auto g = gcd(ans.first, ans.second);
            ans.first /= g;
            ans.second /= g;
        }
        cout << " ";
