...
/////////////////////////////////////////////////////////////////////
// Great Circle Distance - Distncia entre dois pontos em uma esfera
...
int main (){
	...
	while(t--){
		...
		for(int i = 0 ; i < n ; i++){
			...
			sheep.push_back(pt{x, y, i + 1});
		}

		convex_hull_2(sheep, fence);
		...
		for(int i = 1 ; i < fence.size() ; i++){
			s += dist(fence[i], fence[i-1]);
		}

  		printf("%.2lf\n", s);
		//cout << std:: setprecision(5) << s << endl;

		printf("%d", fence[0].pos);
		
		for(int i = 1 ; i < fence.size() - 1 ; i++){
			printf(" %d", fence[i].pos);
		}
